...
import {BasePeriod, BasePeriodType, PeriodTypeCategory, PeriodUtility} from "@hisptz/dhis2-utils";
                        ...
                        {category === PeriodTypeCategory.RELATIVE ? (
                            <div style={{padding: "8px 0"}}>
                                <SingleSelectField
                                    ...
                                    {periodTypes?.map((periodType: BasePeriodType) => (
                                        <SingleSelectOption dataTest={`${periodType?.id}-type`} key={periodType?.id}
                                                            label={periodType.config.name} value={periodType?.id}/>
                                    ))}
                                </SingleSelectField>
                            </div>
                        ) : (
                            <div style={{display: "grid", gridTemplateColumns: "3fr 1fr", gap: 8, padding: "8px 0"}}>
                                ...
                                <div>
                                    <InputField
                                        ...
                                        type={"number"}
                                        value={year}
                                        max={!allowFuturePeriods ? new Date().getFullYear().toString() : undefined}
                                        onChange={({value}: { value: number }) => {

                                            console.log(allowFuturePeriods)
                                            if (!allowFuturePeriods && value > new Date().getFullYear()) {
                                                console.log("Really??")
                                                return;
                                            }
                                            setYear(value)
