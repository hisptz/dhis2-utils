...
export class OfflineOrganisationUnitDB extends Dexie {
    ...
    public updateCount({
                           ...
                           key,
                           value
                       }: { resource: "orgUnits" | "levels" | "groups"; key: "total" | "page"; value: number }) {
    ...
    public isCachingComplete() {
        ...
        return some(Object.values(JSON.parse(cache)), (resourceJson: { total: number, page: number }) => {
            return (resourceJson.total !== 0) && (resourceJson.total === resourceJson.page);
        })
    }
