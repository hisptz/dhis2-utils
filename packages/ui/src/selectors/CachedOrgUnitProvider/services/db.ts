...
export class OfflineOrganisationUnitDB extends Dexie {
	...
	public constructor(name?: string) {
		...
		this.version(2).stores({
			organisationUnits:
				"&id, displayName, parent.id, *organisationUnitGroups",
	...
	public updateCount({
		...
		resource: "orgUnits" | "levels" | "groups";
		key: "total" | "page";
		value: number;
	}) {
	...
	public isCachingComplete() {
		const cache = localStorage.getItem(this.localStorageKey);
		if (cache == null) {
			return false;
		}
		return some(
			Object.values(JSON.parse(cache)),
			(resourceJson: { total: number; page: number }) => {
				return (
					resourceJson.total !== 0 &&
					resourceJson.total === resourceJson.page
				);
			},
		);
	}

	public async addOrganisationUnits(
		organisationUnits: OfflineOrganisationUnit[],
	) {
	...
	public async addOrganisationUnitGroups(
		organisationUnitGroups: OfflineOrganisationUnitGroup[],
	) {
	...
	public async addOrganisationUnitLevels(
		organisationUnitLevels: OfflineOrganisationUnitLevel[],
	) {
