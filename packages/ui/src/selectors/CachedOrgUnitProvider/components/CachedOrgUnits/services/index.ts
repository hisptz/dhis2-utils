...
async function getData(
	instance:
		| Table<OfflineOrganisationUnitGroup>
		| Table<OfflineOrganisationUnitLevel>,
	query: any,
	{
		engine,
	}: {
	...
	if (query.id) {
		const data = head(
			await instance.where("id").equals(query.id).toArray(),
		);
		if (!isEmpty(data)) {
			return data;
		} else {
...
async function getOrgUnitById(
	instance: Table<OfflineOrganisationUnit>,
	query: any,
	engine: any,
) {
	try {
		...
		if (orgUnit) {
			if (!isEmpty(query.params.fields)) {
				...
				const children = await instance
					.where("parent.id")
					.equals(updatedOrgUnit.id)
					.toArray();
		...
		console.warn(`Could not get org unit with id ${query.id} from cache`);
		return await getFromEngine(engine, query);
	}
}

function generateWhereFromFilter(
	instance: Table<OfflineOrganisationUnit>,
	filter: string,
): Collection<OfflineOrganisationUnit, IndexableType> {
	...
	switch (type) {
		case "organisationUnitGroups.id":
			...
			return instance
				.where("organisationUnitGroups")
				.anyOfIgnoreCase(...orgUnitGroups);
		case "identifiable":
			return instance
				.where("id")
...
function filterByStringFilter(
	ou: OfflineOrganisationUnit,
	filter: string,
): boolean {
	const [type, , value] = filter.split(":");
	const orgUnitGroups = [];
	switch (type) {
		case "organisationUnitGroups.id":
			...
			return !isEmpty(
				intersection(orgUnitGroups, ou.organisationUnitGroups),
			);
		case "identifiable":
...
async function getOrgUnits(
	...
	try {
		...
		if (!isEmpty(query.params.filter)) {
			...
			let data;
			if (filters.length === 1) {
				data = await generateWhereFromFilter(
					instance,
					filters[0],
				).toArray();
			} else {
				const collection = generateWhereFromFilter(
					instance,
					filters[0],
				);
				collection.and((ou) => {
		...
		console.warn("Could not get data from cache...");
		return await getFromEngine(engine, query);
	}
}
