...
function sanitizeOrgUnitGroups(
	groups: Array<any>,
): Array<OfflineOrganisationUnit> {
	return groups.map((ou) =>
		set(
			ou,
			"organisationUnitGroups",
			ou.organisationUnitGroups?.map(
				(ouGroup: { id: string }) => ouGroup.id,
			),
		),
	);
}
		...
		if (pageCount > 0) {
			...
			const levels = await mapSeries(
				...
				async (page: number) => {
					...
					]) as OfflineOrganisationUnitLevel[];
				},
			).then(flattenDeep);
			...
			const groups = await mapSeries(
				...
				async (page: number) => {
					...
					return get(data, [
						"groups",
						"organisationUnitGroups",
					]) as OfflineOrganisationUnitGroup[];
				},
			).then(flattenDeep);
	...
	useEffect(() => {
		async function get() {
			...
			try {
				...
				if (!hasLevels) {
					await getLevels({
						pageSize: pageSize ?? OFFLINE_ORG_UNIT_PAGE_SIZE,
					});
				}
				if (!hasGroups) {
					await getGroups({
