...
export function useManageLegendDefinitions(
	...
	const nonDefaultLegendDefinitions =
		getNonDefaultLegendDefinitions(legendDefinitions);
	const defaultLegendDefinitions =
		getDefaultLegendDefinitions(legendDefinitions);

	const onAdd = useCallback(
		(value: any) => {
			const newLegendDefinitions = [
				...cloneDeep(legendDefinitions),
				value,
			];
			...
			if (shouldVerify) {
				confirm({
					...
					message: i18n.t(
						"Adding a legend definition will reset all legends to their default values. Are you sure you want to continue?",
					),
	...
	const onDelete = useCallback(
		(data: { id: string }) => () => {
			...
			const index = findIndex(
				updatedDefinitions,
				({ id: legendDefinitionId }) => legendDefinitionId === data.id,
			);
			...
			if (shouldVerify) {
				confirm({
					...
					message: `${i18n.t(
						"Deleting this definition will reset all configured legends. Are you sure you want to delete this legend definition?",
					)}`,
					confirmButtonColor: "primary",
					confirmButtonText: i18n.t("Reset"),
					onConfirm: () => {
