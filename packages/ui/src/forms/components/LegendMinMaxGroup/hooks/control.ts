...
function normalizeValues(
	...
	const index = findIndex(updatedLegends, [
		"legendDefinitionId",
		value.legendDefinitionId,
	]);
	...
	if (highIsGood) {
		if (nextIndex >= updatedLegends.length) {
			...
			set(updatedLegends, [nextIndex], {
				...updatedLegends[nextIndex],
				endValue: value.startValue,
			});
		}
		if (prevIndex >= 0) {
			set(updatedLegends, [prevIndex], {
				...updatedLegends[prevIndex],
...
export function useControlMinMaxFields({
	...
	const onChange = useCallback(
		(legend: LegendValue) => {
			...
			onFieldChange(
				normalizeValues(updatedList, legend, {
					highIsGood,
					legendDefinitions,
				}),
			);
