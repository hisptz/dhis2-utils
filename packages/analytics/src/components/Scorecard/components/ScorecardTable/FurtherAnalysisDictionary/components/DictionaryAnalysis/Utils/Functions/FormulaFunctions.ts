...
export function getDetailedValueFromApi(engine: any, id: any, type: any) {
	if (type === dataTypes.DATA_ELEMENT) {
		if (isPureDataElement(id)) {
			...
			return new Promise((resolve, reject) => {
				const arr = id.split(".");
				resolve(
					getValueDataElementSourceWithCombo(engine, arr[0], arr[1]),
				);
			});
		}
	}
	if (
		...
		return new Promise((resolve) => {
			const arr = id.split(".");
			resolve(
				getValueProgramDataElementWithSource(engine, arr[0], arr[1]),
			);
		});
	}
