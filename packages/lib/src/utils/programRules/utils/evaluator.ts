    ...
    return programRulesActions.map((programRuleAction) => {
        return {
            ...
            target: {
                ...
                type: programRuleAction.dataElement ? 'DATA_ELEMENT' : 'ATTRIBUTE',
            },
        };
    });
}
...
export function getVariablesFromCondition(condition: string) {
    const ruleVariables = [
        ...flatten(
            Array.from(condition.matchAll(RegularExpressions.VARIABLE)).map((match) =>
                tail(match)
            ) ?? []
        ),
    ];
    const functionParameters = [...(condition.matchAll(RegularExpressions.FUNCTION) ?? [])];
    return uniq([
...
export function translateProgramRule(
    ...
    return {
        ...
        triggers:
            getRuleTriggers(programRule.condition ?? '', programRuleVariables, idPrefix) ?? [],
        id: programRule.id,
    };
}
...
export function replaceConstants(condition: string, variables: BuiltInVariableValueOptions) {
    const constants =
        flatten(
            Array.from(condition.matchAll(RegularExpressions.CONSTANT)).map((match) =>
                tail(match)
            )
        ) ?? [];
    return constants.reduce((condition, constant) => {
        const constantObject: BuiltInVariable | undefined = find(builtInVariables, {
            key: constant,
...
export function executeCondition(
    ...
    const sanitizedCondition = sanitizeConditions(conditionWithConstantsReplacement);
