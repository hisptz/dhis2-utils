...
export const FIXED_PERIOD_TYPES: PeriodTypeInterface[] = [
	...
	},
	{
		...
		rank: 2,
		idGenerator: (interval) =>
			`${interval.start.toFormat("yyyy")}W${interval.start.weekNumber}`,
		nameGenerator: (interval) =>
			`${i18n.t("Week")} ${
				interval.start.weekNumber
			} ${interval.start.toFormat(
				`yyyy-MM-dd`,
	...
	},
	{
		...
		idGenerator: (interval) =>
			`${interval.start.toFormat("yyyy")}WedW${
				interval.start.weekNumber
			}`,
		nameGenerator: (interval) =>
			`${i18n.t("Week")} ${
	...
	},
	{
		...
		idGenerator: (interval) =>
			`${interval.start.toFormat("yyyy")}ThuW${
				interval.start.weekNumber
			}`,
		nameGenerator: (interval) =>
			`${i18n.t("Week")} ${
	...
	},
	{
		...
		idGenerator: (interval) =>
			`${interval.start.toFormat("yyyy")}SatW${
				interval.start.weekNumber
			}`,
		nameGenerator: (interval) =>
			`${i18n.t("Week")} ${
	...
	},
	{
		...
		idGenerator: (interval) =>
			`${interval.start.toFormat("yyyy")}SunW${
				interval.start.weekNumber
			}`,
		nameGenerator: (interval) =>
			`${i18n.t("Week")} ${
	...
	},
	{
		...
		factor: 2,
		idGenerator: (interval) =>
			`${interval.start.toFormat("yyyy")}BiW${Math.ceil(
				interval.start.weekNumber / 2,
			)}`,
		nameGenerator: (interval) =>
			`${i18n.t("Bi-Week")} ${Math.ceil(
				interval.start.weekNumber / 2,
			)} ${interval.start.toFormat(
				`yyyy-MM-dd`,
			)} - ${interval.end.toFormat(`yyyy-MM-dd`)}`,
	},
	{
		...
		rank: 4,
		idGenerator: (interval) =>
			`${interval.start.toFormat("yyyy")}${padStart(
				interval.start.month.toString(),
				2,
				"0",
			)}`,
	...
	},
	{
		...
		nameGenerator: (interval) =>
			`${interval.start.monthLong} - ${
				interval.end.monthLong
			} ${interval.start.toFormat(`yyyy`)}`,
	},
	{
		...
		rank: 6,
		idGenerator: (interval) =>
			`${interval.start.toFormat("yyyy")}Q${interval.start.quarter}`,
		nameGenerator: (interval) =>
			`${interval.start.monthLong} - ${
				interval.end.monthLong
			} ${interval.start.toFormat(`yyyy`)}`,
	},
	{
		...
		factor: 6,
		idGenerator: (interval) =>
			`${interval.start.year}S${Math.ceil(interval.start.quarter / 2)}`,
		nameGenerator: (interval) =>
			`${interval.start.monthLong} - ${
				interval.end.monthLong
			} ${interval.start.toFormat(`yyyy`)}`,
	},
	{
		...
		idGenerator: (interval) =>
			`${interval.start.year}AprilS${Math.ceil(
				interval.start.quarter / 2,
			)}`,
		nameGenerator: (interval) =>
			`${interval.start.monthLong} - ${
				interval.end.monthLong
			} ${interval.start.toFormat(`yyyy`)}`, //TODO: Fix issues with naming of the periods when they are of different years
	},
	{
	...
	},
	{
		...
		nameGenerator: (interval) =>
			`${interval.start.toFormat(`MM yyyy`)} - ${interval.end.toFormat(
				`MM yyyy`,
			)}`,
	},
	{
	...
	},
	{
		...
		nameGenerator: (interval) =>
			`${interval.start.toFormat(`MMMM yyyy`)} - ${interval.end.toFormat(
				`MMMM yyyy`,
			)}`,
	},
];
