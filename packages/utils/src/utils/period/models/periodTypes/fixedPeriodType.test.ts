...
const fixedPeriodsTests = [
	...
	},
	{
		...
		periodNameTest:
			/Week ([0-9]+) ([0-9]{4})-([0-9]{2})-([0-9]{2}) - ([0-9]{4})-([0-9]{2})-([0-9]{2})/,
		noOfPeriods: 52,
	...
	},
	{
		...
		periodNameTest:
			/Bi-Week ([0-9]+) ([0-9]{4})-([0-9]{2})-([0-9]{2}) - ([0-9]{4})-([0-9]{2})-([0-9]{2})/,
...
describe("Fixed Period Test", () => {
	fixedPeriodsTests.forEach((test) => {
		it(`${test.description}`, () => {
			const periodType = FixedPeriodType.getFromId(test.id, {
				year,
				preference,
			});
			...
			periodIds.forEach((periodId) => {
				expect(
					(periodId.match(test.periodIdTest)?.length ?? 0) > 1,
				).toBe(true);
			});
			periodNames.forEach((periodName) => {
				expect(
					(periodName.match(test.periodNameTest)?.length ?? 0) > 1,
				).toBe(true);
			});
