...
export class RelativePeriod extends BasePeriod {
	...
	constructor(
		{ id, name, offset = 0, span }: RelativePeriodConfig,
		type: PeriodTypeInterface,
	) {
		super();
		const startDate = DateTime.local()
			.startOf(type.unit)
			.minus({ [type.unit]: offset })
			.minus({ [type.unit]: span })
			.minus({ [type.unit]: type.factor });
	...
	static getById(id: string): RelativePeriod {
		...
		].find((periodType: PeriodTypeInterface) => {
			if (periodType.getPeriods) {
				return periodType
					.getPeriods()
					.find((periodObject) => periodObject.id === id);
			}
		});
		if (!periodTypeConfig) {
