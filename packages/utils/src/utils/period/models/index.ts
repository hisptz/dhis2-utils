...
export class FixedPeriods extends PeriodCategory {
	get periodTypes(): BasePeriodType[] {
		return FIXED_PERIOD_TYPES.map(
			(periodTypeConfig) =>
				new FixedPeriodType(periodTypeConfig, {
					year: this.year,
...
export class RelativePeriods extends PeriodCategory {
	get periodTypes(): BasePeriodType[] {
		return RELATIVE_PERIOD_TYPES.map(
			(periodTypeConfig) =>
				new RelativePeriodType(
					periodTypeConfig,
					this.preference ?? { allowFuturePeriods: false },
				),
		);
	}
}
 ...
 * */
export function initializePeriodUtility(
