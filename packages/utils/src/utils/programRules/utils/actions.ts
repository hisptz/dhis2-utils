...
function joinHideOptionGroupActions(
	actions: RunnableAction[],
): RunnableAction | undefined {
...
function joinShowOptionGroupActions(
	actions: RunnableAction[],
): RunnableAction | undefined {
	const hideOptionGroupActions = actions.filter((action) =>
...
function joinHideOptionActions(
	actions: RunnableAction[],
): RunnableAction | undefined {
...
function joinShowOptionActions(
	actions: RunnableAction[],
): RunnableAction | undefined {
	const hideOptionsActions = actions.filter((action) =>
...
function joinShowWarningActions(
	actions: RunnableAction[],
): RunnableAction | undefined {
	const showWarningActions = actions.filter(
		(action) => action.type === "SHOWWARNING",
	);
...
function joinShowErrorActions(
	actions: RunnableAction[],
): RunnableAction | undefined {
	const showErrorActions = actions.filter(
		(action) => action.type === "SHOWERROR",
	);
...
function joinDisableFieldActions(
	actions: RunnableAction[],
): RunnableAction | undefined {
	const disableFieldActions = actions.filter(
		(action) => action.type === "DISABLEFIELD",
	);
...
function joinMinMaxActions(
	actions: RunnableAction[],
): RunnableAction | undefined {
	const minMaxActions = actions.filter(
		(action) => action.type === "SETMINMAXVALUE",
	);
...
function disableFields(
	actions: RunnableAction[],
	{ toggleFieldDisabled }: ActionCallbacks,
) {
...
export function runOptionsAction(
	fields: RunnableAction[],
	callbacks: ActionCallbacks,
) {
...
export async function manageOptionGroups(
	...
	const sanitizeFields = Object.values(
		mapValues(groupedFields, (field, fieldId) => {
			return {
				field: fieldId,
				optionGroups: flatten(
					field.map(({ optionGroups }) => optionGroups),
				),
			};
		}) as any,
	);
	toggleLoading(
		Object.keys(groupedFields).map((fieldId) => ({
			field: fieldId,
			loading: true,
		})),
	);
	const promises = sanitizeFields.map(
		async (field: any) =>
			await getOptionsToHide(field, { getOptionGroups }, hide),
	);
